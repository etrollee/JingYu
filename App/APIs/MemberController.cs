...
namespace App.APIs
{
    public class MemberController : ApiController
    {
        ...
        public MemberController()
            : this(new ClientManagerBLL(), new InformationsBLL(), new MerchantBLL(),new ServiceProductBLL())
        {

        }

        public MemberController(ClientManagerBLL clientManagerBLL, InformationsBLL informationBll,
            MerchantBLL merchantBll,ServiceProductBLL serviceProductBll)
        {
        ...
        public HttpResponseMessage Appointment(int type,string memberId, int id)
        {
            try
            {
                ...
                if (type==2)
                {
                      var product = _iServiceProductBll.GetById(id);
                      if (product.MerchantId==memberId)
                      {
                          return Request.CreateResponse(HttpStatusCode.OK, 
                              new {success=false,msg="商家不能预约自己的产品",overInfo="" });
                      }
                }
