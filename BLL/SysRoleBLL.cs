...
namespace BLL
{
    ...
    public class SysRoleBLL : IBLL.ISysRoleBLL, IDisposable
    {
        ...
        /// <returns></returns>
        public bool CheckName(SysRole entity)
        {
            return db.SysRole.Any(o => o.Name == entity.Name && o.Id!=entity.Id);
        }
        ...
        public bool Create(ref ValidationErrors validationErrors, SysEntities db, SysRole entity)
        {
            ...
            if (CheckName(entity))
            {
                validationErrors.Add("角色名称已被占用，请换一个新的角色名称");
                return false;
            }
        ...
        public bool DeleteCollection(ref ValidationErrors validationErrors, string[] deleteCollection)
        {
            try
            {
                ...
                {
                    var operationCount = GetSysMenuSysRoleSysOperation(deleteCollection[0]);
                    if (operationCount > 0)
                    {
                        validationErrors.Add("该角色已经分配了权限，不可删除，若要删除请先删除该角色下的权限！");
                        return false;
                    }
                    var rolePersonCount = GetSysRoleSysPerson (deleteCollection[0]);
                    if (rolePersonCount > 0)
                    {
                        validationErrors.Add("该角色已经设置了用户，不可删除，若要删除请先删除该角色下的用户关系！");
                        return false;
                    }
                    using (TransactionScope transactionScope = new TransactionScope())
                    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public int GetSysMenuSysRoleSysOperation(string id)
        {
            using (var dataContext = new SysEntities())
            {
                int count = 0;
                string sql = @"
                    SELECT  COUNT(SysRoleId) AS Count
                    FROM    dbo.SysMenuSysRoleSysOperation
                    WHERE   SysRoleId = @SysRoleId";
                SqlCommand scmd = new SqlCommand(sql);
                scmd.Parameters.AddWithValue("@SysRoleId", id);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public int GetSysRoleSysPerson(string id)
        {
            using (var dataContext = new SysEntities())
            {
                int count = 0;
