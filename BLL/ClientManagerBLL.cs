...
namespace BLL
{
    public class ClientManagerBLL : IClientManagerBLL, IDisposable
    {
        ...
        public List<int> GetUnChangeMessageCount(int memberType,string memberId, int type, int update)
        {
            using (var db = new SysEntities())
            {
                ...
                if (memberType==1)
                {
                    ...
                    SELECT  COUNT(MInfo.Id) AS total
					FROM    dbo.MemberInformations MInfo
                    INNER JOIN Informations INFO ON MInfo.InformationsId=INFO.Id
					WHERE   IsFeedback = 0
							AND MemberId = @MemberId
							AND InformationsId IN (
							SELECT  InformationId
							FROM    InformationFeedbackTemplate IFT
									INNER JOIN dbo.MemberInformations MI ON MI.InformationsId = IFT.InformationId
							WHERE   MemberId = @MemberId )
                            AND INFO.TimeLimit>DATEDIFF(HOUR,INFO.CreateTime,GETDATE())";
                }
                else
                {
                    selectSql = @"
                    ...
                    INNER JOIN Informations INFO ON MInfo.InformationsId=INFO.Id
					WHERE   IsFeedback = 0
							AND MerchantId = @MemberId
							AND InformationsId IN (
							SELECT  InformationId
							FROM    InformationFeedbackTemplate IFT
									INNER JOIN dbo.MerchantInformations MI ON MI.InformationsId = IFT.InformationId
                ...
                try
                {
                    ...
                    if (type==1)
                    {
                        sql = @"DELETE FROM InformationMember WHERE MemberId=@MemberId AND InformationId=@InformationId
                                ...
                                DELETE FROM InformationFeedback WHERE MemberId=@MemberId AND InformationsId=@InformationId";
                    }
                    else
                    {
                ...
                try
                {
                    ...
                    if (type==1)
                    {
                        sql = @"DELETE FROM InformationMember WHERE MemberId=@MemberId 
                                ...
                                DELETE FROM InformationFeedback WHERE MemberId=@MemberId";
                    }
                    else
                    {
        ...
        /// <returns></returns>
        public Merchant GetByUpdateFlag(Guid updateFlag)
        {
            using (SysEntities db = new SysEntities())
            {
                return db.Merchant.SingleOrDefault(o => o.UpdateFlag == updateFlag);
            }
        }
        ...
        public int AddAppointment(int type,Appointment appointment)
        {
            using (var db = new SysEntities())
            {
                ...
                else
                {
                    ...
                    db.AddToMerchantInformations(merchantInfo);
                        ...
                        var arg = new DbParameter[] 
                                { 
                                     ...
                                     new SqlParameter{ParameterName="Question",
                                          Value=informationFeedback.Question==null?"":informationFeedback.Question},
