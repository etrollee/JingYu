...
namespace BLL
{
    public class InformationsBLL : IInformationsBLL, IDisposable
    {
        ...
        {
            string sql = @"INSERT INTO MemberInformations(InformationsId,MemberId,IsRead,IsFeedback)
                                VALUES(@InformationsId,@MemberId,@IsRead,@IsFeedback)";
            var args = new DbParameter[] 
                        { 
                             ...
                             new SqlParameter{ ParameterName="IsRead",Value=model.IsRead},
                             new SqlParameter{ ParameterName="IsFeedback",Value=model.IsFeedback},
                        };
            db.ExecuteStoreCommand(sql, args);
        }
        ...
        /// <param name="db"></param>
        /// <param name="model"></param>
        private void AddToMerchantInformations(SysEntities db, MerchantInformations model)
        {
            string sql = @"INSERT INTO MerchantInformations(InformationsId,MerchantId,IsRead,IsFeedback)
                                VALUES(@InformationsId,@MerchantId,@IsRead,@IsFeedback)";
            var args = new DbParameter[] 
                        { 
                             new SqlParameter { ParameterName = "InformationsId", Value =model.InformationsId},
                             new SqlParameter { ParameterName = "MerchantId", Value =model.MerchantId},
                             new SqlParameter{ ParameterName="IsRead",Value=model.IsRead},
                             new SqlParameter{ ParameterName="IsFeedback",Value=model.IsFeedback},
                        };
            db.ExecuteStoreCommand(sql, args);
            ...
            using (var repository = new InformationsRepository())
            {
                try
                {
                    ...
                    if (merchants.Count > 0)
                    {
                        ...
                        {
                            var merchantInfo = new MerchantInformations
                            {
                                InformationsId = entity.Id,
                                MerchantId = item.Id,
                                IsRead = false,
                            ...
                            AddToMerchantInformations(db, merchantInfo);
                        }
                    }
                    return true;
                }
        ...
        public bool Create(ref ValidationErrors validationErrors, SysEntities db, Informations entity)
        {
            using (var repository = new InformationsRepository())
            {
                try
                {
                    ...
                    //if (entity.FeedbackTemplateId.GetIdSort().Count==0)
                    //{
                    //    validationErrors.Add("： 态度反馈模板不能为空！");
                    //    return false;
                    //}
                     if (entity.MemberId.GetIdSort().Count()== 0 && entity.MerchantId.GetIdSort().Count()==0)
                    {
                        validationErrors.Add("： 会员或商家不能为空！");
                        return false;
                    }
